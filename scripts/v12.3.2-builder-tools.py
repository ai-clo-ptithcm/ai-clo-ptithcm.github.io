...
s=s.replace('/* AI-CLO PTITHCM V12.3 — Online Assessment Builder module. */','/* AI-CLO PTITHCM V12.3.2 — Online Assessment Builder module. */',1)
s=s.replace('          frozenPool: new Map(),\n          selectionDirty: false,','          frozenPool: new Map(),\n          draftOverrides: new Map(),\n          selectionDirty: false,',1)
s=s.replace('        for (const [id, q] of ctx.frozenPool || []) merged.set(id, q);\n        return [...merged.values()].filter(', '        for (const [id, q] of ctx.frozenPool || []) merged.set(id, q);\n        for (const [id, q] of ctx.draftOverrides || []) merged.set(id, q);\n        return [...merged.values()].filter(',1)
s=s.replace('        bindBuilder(ctx);\n      }\n      function builderInfo(ctx) {','        bindBuilder(ctx);\n        renderMathIn(c);\n      }\n      function renderMathIn(root) {\n        if (typeof window.renderMath !== "function") return;\n        requestAnimationFrame(() => window.renderMath(root || getAssessmentRoot()));\n      }\n      function builderInfo(ctx) {',1)

start=s.index('      function questionCard(ctx, q, index) {')
end=s.index('      function builderQuestions(ctx) {', start)
replacement=r'''      function questionCode(ctx, q) {
...
anchor='''      function replaceSelectedQuestion(ctx, index) {
        const pool = replacementCandidates(ctx, index);'''
insert=r'''      function closeBuilderDrawer() {
...
'''
assert anchor in s, 'replace anchor missing'
s=s.replace(anchor, insert+anchor,1)

old='''        qsa("[data-v122-replace]", c).forEach(
          (el) =>
            (el.onclick = () =>
              replaceSelectedQuestion(ctx, +el.dataset.v122Replace)),
        );'''
new='''        qsa("[data-v122-replace]", c).forEach(
          (el) =>
            (el.onclick = () =>
              replaceSelectedQuestion(ctx, +el.dataset.v122Replace)),
        );
...
assert old in s, 'binding anchor missing'
s=s.replace(old,new,1)

old='''          renderBuilder(ctx);
        });
        qs("#v122AiUse")?.addEventListener'''
new='''          renderBuilder(ctx);
        });
        renderMathIn(document.querySelector("#modal"));
        qs("#v122AiUse")?.addEventListener'''
assert old in s, 'Gemini preview anchor missing'
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')

p=Path('css/exams/unified-builder.css')
css=p.read_text(encoding='utf-8')
css += '\n/* V12.3.2 Builder question tools */\n.ub-question-code{font:600 .78rem/1.2 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;color:var(--muted,#667085);background:var(--soft,#f8fafc);border:1px solid var(--border,#e4e7ec);border-radius:6px;padding:4px 6px}.ub-question-actions{justify-content:flex-end}.ub-picker-list{display:grid;gap:9px;margin-top:12px}.ub-picker-list article{display:grid;gap:8px;padding:11px;border:1px solid var(--border,#e4e7ec);border-radius:10px;background:#fff}.ub-picker-list article>div:first-child{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.ub-picker-content{font-weight:600}.ub-picker-list article button{justify-self:start}.ub-quick-edit{display:grid;gap:12px}.ub-edit-options{display:grid;gap:9px}.ub-edit-options label{display:grid;grid-template-columns:44px 1fr;gap:8px;align-items:start}.ub-edit-options label>span{padding-top:8px}.ub-edit-options textarea{width:100%}@media(max-width:800px){.ub-question-actions{justify-content:flex-start}.ub-picker-list article button{width:100%}.ub-edit-options label{grid-template-columns:36px 1fr}}\n'
p.write_text(css,encoding='utf-8')

p=Path('app.html')
a=p.read_text(encoding='utf-8')
a=a.replace('css/exams/unified-builder.css?v=12.2.0','css/exams/unified-builder.css?v=12.3.2',1)
a=a.replace('js/assessment/online-builder.js?v=12.3.1','js/assessment/online-builder.js?v=12.3.2',1)
