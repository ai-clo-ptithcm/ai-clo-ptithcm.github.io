/* AI-CLO PTITHCM v10.5.3 — class membership page for Admin. */
(() => {
'use strict';
...
window.users=async function(c){
 ...
 const teachers=profiles.filter(p=>teacherRoles.includes(p.role)).length;
 const students=profiles.filter(p=>p.role==='student').length;
 const admins=profiles.filter(p=>p.role==='admin').length;
 const locked=profiles.filter(p=>p.is_active===false).length;
 c.innerHTML=`<div class="stats v1053-class-stats">
   <div class="stat"><small>Tổng tài khoản</small><b>${total}</b></div>
