...
window.showDraft=function(batch,drafts,d,pos){
 ...
 const approve=$('#approveDraft');if(approve?.onclick){const save=approve.onclick;approve.onclick=async event=>{const originalRpc=db.rpc.bind(db),originalFrom=db.from.bind(db);db.rpc=(name,params)=>name==='find_similar_questions'?originalRpc('find_similar_questions_scoped',{p_subject_id:params.p_subject_id,p_chapter_id:batch.chapter_id,p_topic_id:d.topic_id||batch.topic_id||null,p_content:params.p_content,p_exclude_id:params.p_exclude_id||null,p_limit:params.p_limit||3}):originalRpc(name,params);db.from=table=>{const builder=originalFrom(table);if(table==='questions'){const insert=builder.insert.bind(builder);builder.insert=(values,...args)=>insert({...values,origin_type:'gemini',ai_batch_id:batch.id},...args)}return builder};try{return await save.call(approve,event)}finally{db.rpc=originalRpc;db.from=originalFrom}}}
