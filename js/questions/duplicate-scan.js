/* AI-CLO PTITHCM V11.6.12 — in-page AI duplicate scan with filters and edit-return state. */
(()=>{
'use strict';
...
function pairHtml(pair,index,sets){
 ...
 const card=q=>`<article class="duplicate-question-card"><div class="duplicate-question-head"><div><b>${esc(q.code||'—')}</b><small>${questionMeta(q,sets)}</small></div><button type="button" class="secondary" data-edit-duplicate="${esc(q.id)}" data-pair-anchor="${esc(pair.pair_id)}">Sửa câu này</button></div><div class="duplicate-question-content">${esc(q.content||'')}</div></article>`;
