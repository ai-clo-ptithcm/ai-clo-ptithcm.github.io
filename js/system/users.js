/* AI-CLO PTITHCM V12.6.10 — system-wide account management. */
(() => {
'use strict';
...
/* Giữ đúng trang con của Quản lý người dùng khi chuyển sang mục khác rồi quay lại.
   Dùng sessionStorage để trạng thái chỉ tồn tại trong phiên trình duyệt hiện tại. */
...
function createPage(c,onBack){
 ...
 const single=()=>{
  ...
  form.onsubmit=async e=>{e.preventDefault();let b=$('#inlineCreateUser'),v=Object.fromEntries(new FormData(e.target));b.disabled=true;b.textContent='Đang tạo…';try{showAccountResults(await createAccounts([{full_name:v.full_name.trim(),email:v.email.trim(),role:v.role,mssv:v.mssv.trim()||null}]));writeUserPageState({page:'create',tab:'single',draft:{}})}catch(x){err(x);b.disabled=false;b.textContent='Tạo tài khoản'}};
 };
 const csv=()=>{
  activate('csv');writeUserPageState({page:'create',tab:'csv'});
  $('#userCreateBody').innerHTML=`<form id="inlineCsvUsers" class="form-grid user-inline-form"><div class="csv-guide wide"><b>Cấu trúc CSV</b><code>full_name,email,role,mssv</code><span>Vai trò dùng <b>admin</b>, <b>teacher</b> hoặc <b>student</b>. Admin có quyền toàn hệ thống. Tối đa 100 tài khoản mỗi lần.</span></div><label class="field wide">Chọn file CSV<input id="inlineCsvFile" type="file" accept=".csv,text/csv" required></label><div id="inlineCsvPreview" class="wide"></div><div class="form-actions"><button class="primary" id="inlineCsvSubmit">Tạo các tài khoản</button></div></form>`;
  let parsed=[];$('#inlineCsvFile').onchange=async e=>{try{parsed=parseCsv(await e.target.files[0].text());if(parsed.length>100)throw new Error('File có quá 100 người');$('#inlineCsvPreview').innerHTML=`<span class="badge green">Đã đọc ${parsed.length} tài khoản</span>`}catch(x){parsed=[];err(x)}};
...
function managePage(c,p,members,onBack){
 heading();writeUserPageState({page:'manage',userId:p.id});let joined=members.filter(m=>m.user_id===p.id).map(m=>({m,s:state.subjects.find(x=>x.id===m.subject_id)})).filter(x=>x.s);
 c.innerHTML=`<div class="user-subpage-head">${back()}<div><h3>Quản lý tài khoản</h3><p>Thông tin, học phần và quyền truy cập của người dùng.</p></div></div><section class="panel user-account-page"><div class="detail-identity"><div class="avatar">${esc((p.full_name||p.email||'?').trim()[0].toUpperCase())}</div><div><h3>${esc(p.full_name||'Chưa đặt tên')}</h3><p>${esc(p.email||'')}${p.mssv?` · ${esc(p.mssv)}`:''}</p><span class="badge ${p.role==='student'?'green':'red'}">${esc(roleText(p.role))}</span> <span class="badge ${p.is_active===false?'red':'green'}">${p.is_active===false?'Đã khóa':'Hoạt động'}</span></div></div><div class="user-account-actions"><button id="pageEditUser" class="secondary">Sửa thông tin</button><button id="pageResetPassword" class="secondary">Đặt lại mật khẩu</button>${p.role!=='admin'&&p.id!==state.user.id?`<button id="pageToggleLock" class="${p.is_active===false?'primary':'danger'}">${p.is_active===false?'Mở khóa':'Khóa tài khoản'}</button>`:''}</div><div id="pagePasswordResult"></div><div class="panel-head user-course-head"><div><h3>Học phần tham gia</h3><p class="hint">Một người dùng có thể tham gia nhiều học phần.</p></div><span class="badge">${p.role==='admin'?'Toàn hệ thống':joined.length+' học phần'}</span></div><div class="subject-memberships">${p.role==='admin'?'<div><b>Toàn hệ thống</b><small>Quản trị viên · không thể hạ quyền, khóa hoặc xóa bởi Admin khác</small></div>':joined.map(x=>`<div><b>${esc(x.s.name)}</b><small>${esc(x.s.semester||'')} · ${esc(x.s.academic_year||'')} · ${esc(roleText(x.m.role))}</small></div>`).join('')||'<p class="hint">Chưa tham gia học phần nào.</p>'}</div></section>`;
 $('#userPageBack').onclick=()=>{clearUserPageState();onBack()};
...
window.AICLO_SYSTEM_USERS=Object.freeze({version:'12.6.10'});
})();