/* AI-CLO PTITHCM V12.6.18 — canonical system dashboard owner and dashboard router. */
(() => {
'use strict';
...
async function renderSystemDashboard(c){
 ...
 if(r==='admin'){
  ...
  cards=[
   ['Học phần đang diễn ra',ongoingCourses(state.subjects).length],
   ['Ngân hàng câu hỏi',bankCount],
   ['Người dùng',profiles.length],
   ['Giảng viên',profiles.filter(p=>isTeacher(p.role)).length],
   ['Sinh viên',profiles.filter(p=>p.role==='student').length]
  ];
...
window.AICLO_SYSTEM_DASHBOARD=Object.freeze({render:renderSystemDashboard,route:renderDashboard});
})();
