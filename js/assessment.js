/* AI-CLO PTITHCM V12.2 — Assessment single-owner engine.
   Step 7: lifecycle + atomic builder + Student Attempt + teacher preview + official CLO results. */
(()=>{
'use strict';

let liveTimer=null;
const VERSION='12.2.0-step7';
...
async function loadExamAttempts(examId){
  const {data,error}=await db.from('exam_attempts')
    .select('id,exam_id,student_id,attempt_number,started_at,submitted_at,score,profiles:student_id(id,full_name,mssv,email)')
    .eq('exam_id',examId).order('started_at',{ascending:false});
  if(error)throw error;
  return data||[];
}
function attemptStudentName(a){return a.profiles?.full_name||a.profiles?.email||'Sinh viên'}
function attemptSort(rows,mode){
  const out=[...rows];
  if(mode==='name')out.sort((a,b)=>attemptStudentName(a).localeCompare(attemptStudentName(b),'vi'));
  else if(mode==='score')out.sort((a,b)=>(b.score==null?-Infinity:+b.score)-(a.score==null?-Infinity:+a.score));
  else out.sort((a,b)=>new Date(b.submitted_at||b.started_at||0)-new Date(a.submitted_at||a.started_at||0));
  return out;
}
...
async function openExamDetail(examOrId){
  try{
    ...
    const attempts=await loadExamAttempts(exam.id),c=$1('#content');if(!c)return;
    ...
    $1('#v122Preview')?.addEventListener('click',()=>openTeacherPreview(exam));
...
function bindAttemptTable(exam,attempts,mode){
  const box=$1('#v122AttemptList');if(!box)return;
  const sort=$1('#v122AttemptSort',box);if(sort)sort.onchange=()=>{box.innerHTML=attemptTableHtml(attempts,sort.value);bindAttemptTable(exam,attempts,sort.value)};
  $$1('[data-v122-view-attempt]',box).forEach(b=>b.onclick=()=>openTeacherAttemptResult(exam,b.dataset.v122ViewAttempt));
  $$1('[data-v122-delete-attempt]',box).forEach(b=>b.onclick=()=>deleteAttempt(exam,b.dataset.v122DeleteAttempt));
}
async function deleteAttempt(exam,attemptId){
  try{
    if(!await ask('Xóa lượt làm','Xóa lượt làm này? Điểm và dữ liệu trả lời của lượt sẽ bị xóa.','Xóa lượt'))return;
    const r=await db.from('exam_attempts').delete().eq('id',attemptId);if(r.error)throw r.error;
    notify('Đã xóa lượt làm');await openExamDetail(exam.id);
  }catch(e){showError(e)}
}
async function openTeacherAttemptResult(exam,attemptId){
  try{
    ...
    if(typeof openDrawer==='function')openDrawer(`Bài làm · ${exam.title}`,html,null,{wide:true,eyebrow:'GIẢNG VIÊN XEM BÀI'});else notify(`Điểm: ${Number(data.score||0).toFixed(2)}`);
  }catch(e){showError(e)}
}
async function renderDesignSummary(exam){
  const box=$1('#v122DesignSummary');if(!box)return;
  const [{data:pool,error},{data:selected,error:se}]=await Promise.all([
    db.from('exam_question_pool').select('question_id,chapter_name,topic_name,clo_code').eq('exam_id',exam.id),
    db.from('exam_questions').select('question_id,question_order').eq('exam_id',exam.id).order('question_order')
  ]);
  if(error||se){box.innerHTML='<p class="hint">Chưa đọc được snapshot bộ câu.</p>';return}
  const rows=pool||[],clo={};for(const r of rows)clo[r.clo_code||'—']=(clo[r.clo_code||'—']||0)+1;
  box.innerHTML=`<div class="detail-grid"><div><small>Pool đóng băng</small><b>${rows.length} câu</b></div><div><small>Bộ câu mẫu</small><b>${(selected||[]).length} câu</b></div><div><small>CLO trong pool</small><b>${Object.entries(clo).map(([k,v])=>`${esc2(k)}: ${v}`).join(' · ')||'—'}</b></div></div>`;
}
async function loadTeacherPreviewQuestions(exam){
  const [{data:chosen,error:ce},{data:pool,error:pe}]=await Promise.all([
    db.from('exam_questions').select('question_id,question_order').eq('exam_id',exam.id).order('question_order'),
  ...
  let qs=(chosen||[]).map(x=>map.get(x.question_id)).filter(Boolean).map(x=>({...x,options:[...(x.options||[])]}));
  if(exam.shuffle_questions)qs=shuf(qs);
  if(exam.shuffle_options)qs=qs.map(q=>({...q,options:shuf(q.options||[])}));
  return qs;
}
async function openTeacherPreview(exam){
  try{
    const questions=await loadTeacherPreviewQuestions(exam);if(!questions.length)return notify('Bài chưa có bộ câu mẫu để làm thử.',true);
    showTeacherPreviewQuestion(exam,questions,{},0,true);
  }catch(e){showError(e)}
}
function showTeacherPreviewQuestion(exam,questions,answers,index,first=false){
  const qx=questions[index];if(!qx)return;
  ...
  const bind=()=>{
    $$1('input[name="v122PreviewAnswer"]',$1('#drawerBody')).forEach(r=>r.onchange=()=>{answers[qx.question_id]=r.value;showTeacherPreviewQuestion(exam,questions,answers,index,false)});
    $$1('[data-v122-preview-jump]',$1('#drawerBody')).forEach(b=>b.onclick=()=>showTeacherPreviewQuestion(exam,questions,answers,+b.dataset.v122PreviewJump,false));
    $1('#v122PreviewPrev').onclick=()=>showTeacherPreviewQuestion(exam,questions,answers,index-1,false);
    $1('#v122PreviewNext').onclick=()=>showTeacherPreviewQuestion(exam,questions,answers,index+1,false);
    $1('#v122PreviewSubmit').onclick=async()=>{const missing=questions.filter(q=>!answers[q.question_id]).length;if(missing&&!await ask('Nộp bài thử',`Còn ${missing} câu chưa trả lời. Vẫn xem kết quả?`,'Xem kết quả'))return;showTeacherPreviewResult(exam,questions,answers)};
  };
  if(first&&typeof openDrawer==='function')openDrawer(`Làm thử · ${exam.title}`,html,bind,{wide:true,eyebrow:'CHẾ ĐỘ GIẢNG VIÊN'});
  else if(typeof replaceDrawer==='function')replaceDrawer(`Làm thử · ${exam.title}`,html,bind,{wide:true,eyebrow:'CHẾ ĐỘ GIẢNG VIÊN'});
}
function showTeacherPreviewResult(exam,questions,answers){
  const correct=questions.filter(q=>answers[q.question_id]===q.correct_answer).length,codes=[...new Set(questions.map(q=>q.clo_code).filter(Boolean))];
  const byClo=codes.map(code=>{const qs=questions.filter(q=>q.clo_code===code),right=qs.filter(q=>answers[q.question_id]===q.correct_answer).length;return {code,total:qs.length,correct:right,score:qs.length?right*10/qs.length:0}});
...
async function openExamBuilder(exam){
  try{
    const ctx=defaultBuilder(exam);ctx.sets=await loadPracticeSets();
    if(!ctx.sets.chapters.length||!ctx.sets.clos.length)return notify('Học phần cần có Chương và CLO trước khi tạo bài.',true);
    if(!ctx.sets.questions.length)return notify('Ngân hàng luyện tập chưa có câu hỏi được duyệt.',true);
    if(exam){
      const [{count,error},{data:chosen,error:qe},{data:pool,error:pe}]=await Promise.all([
        db.from('exam_attempts').select('id',{count:'exact',head:true}).eq('exam_id',exam.id),
        db.from('exam_questions').select('question_id,question_order').eq('exam_id',exam.id).order('question_order'),
        db.from('exam_question_pool').select('question_id,chapter_id,chapter_name,topic_id,topic_name,clo_id,clo_code,content,correct_answer,explanation,options').eq('exam_id',exam.id)
      ]);
      if(error)throw error;if(qe)throw qe;if(pe)throw pe;
      ctx.locked=(count||0)>0;
      const frozen=new Map((pool||[]).map(row=>[row.question_id,snapshotQuestion(row)]));
      ctx.frozenPool=frozen;
      ctx.selected=(chosen||[]).map(x=>frozen.get(x.question_id)||byId(ctx.sets.questions,x.question_id)).filter(Boolean);
    }
    renderBuilder(ctx);
  }catch(e){showError(e)}
}
...
function optionMap(q){return Object.fromEntries((q.question_options||[]).map(o=>[String(o.option_key||'').toUpperCase(),o.content||'']))}
function questionCard(ctx,q,index){const opts=optionMap(q),clo=byId(ctx.sets.clos,q.clo_id),ch=byId(ctx.sets.chapters,q.chapter_id),tp=byId(ctx.sets.topics,q.topic_id);return `<article class="ub-question-card v122-question-card"><div class="ub-question-head"><div><b>Câu ${index+1}</b><span class="badge red">${esc2(clo?.code||'—')}</span><span class="badge">${esc2(ch?.name||'—')}</span><span class="badge">${esc2(tp?.name||'—')}</span></div>${ctx.locked?'':`<div><button type="button" class="secondary compact" data-v122-replace="${index}">Đổi câu</button><button type="button" class="ai-btn compact" data-v122-ai="${index}">✦ Gemini sinh câu</button></div>`}</div><div class="detail-question">${esc2(q.content||'')}</div><div class="detail-options">${['A','B','C','D'].map(k=>`<div class="${String(q.correct_answer||'').toUpperCase()===k?'correct':''}"><b>${k}</b><span>${esc2(opts[k]||'')}</span></div>`).join('')}</div>${q.explanation?`<p class="hint"><b>Lời giải:</b> ${esc2(q.explanation)}</p>`:''}</article>`}
function builderQuestions(ctx){return `<section class="panel"><div class="panel-head"><div><h3>3. Bộ câu mẫu</h3><p class="hint">Câu đang hiển thị của bài cũ được đọc từ snapshot đóng băng. Đổi câu/Gemini chỉ sửa bản nháp; bấm Lưu mới thay snapshot trong DB.</p></div></div>${ctx.selected.length?`<div class="v122-selected-summary"><b>${ctx.selected.length}/${matrixTotal(ctx)} câu đã rút</b> · ${Object.entries(cloCountsFromQuestions(ctx.selected,ctx.sets)).map(([k,v])=>`${esc2(k)}: ${v}`).join(' · ')}${ctx.selectionDirty?' · <span class="badge">Chưa lưu thay đổi câu</span>':''}</div><div class="v122-selected-list">${ctx.selected.map((q,i)=>questionCard(ctx,q,i)).join('')}</div>`:'<div class="empty"><b>Chưa rút câu</b><span>Hoàn tất ma trận rồi nhấn “Rút câu hỏi”.</span></div>'}</section>`}
...
function replacementCandidates(ctx,index){const old=ctx.selected[index],used=new Set(ctx.selected.map(q=>q.id));used.delete(old?.id);return ctx.sets.questions.filter(q=>q.id!==old?.id&&!used.has(q.id)&&q.clo_id===old?.clo_id&&(ctx.structureMode==='topic_clo'?q.topic_id===old?.topic_id:(q.chapter_id===old?.chapter_id&&ctx.selectedTopics.has(q.topic_id))))}
function replaceSelectedQuestion(ctx,index){const pool=replacementCandidates(ctx,index);if(!pool.length)return notify('Không còn câu khác phù hợp với đúng ô ma trận này.',true);const picked=shuf(pool)[0];ctx.selected[index]=ctx.frozenPool?.get(picked.id)||picked;ctx.selectionDirty=true;renderBuilder(ctx);notify(`Đã đổi Câu ${index+1}. Bấm Lưu thay đổi để cập nhật snapshot.`)}
async function generateAiQuestion(ctx,index,button){
  const old=ctx.selected[index];if(!old)return;
  if(button){button.disabled=true;button.textContent='✦ Đang sinh…'}
  try{
    const {data,error}=await db.functions.invoke('generate-one-question',{body:{subject_id:subjectId(),chapter_id:old.chapter_id,topic_id:old.topic_id,clo_id:old.clo_id}});
    if(error)throw error;if(!data?.success||!data?.question)throw new Error(data?.error||'Gemini không tạo được câu hỏi');
    previewAiQuestion(ctx,index,data.question,data.model||'Gemini');
  }catch(e){showError(e);if(button){button.disabled=false;button.textContent='✦ Gemini sinh câu'}}
}
function previewAiQuestion(ctx,index,g,model){
  ...
  modal(`AI-CLO | Câu ${index+1} do Gemini đề xuất`,`<div class="v122-ai-preview"><p class="hint">${esc2(model)} · Câu chỉ được đưa vào bài sau khi bạn chọn “Dùng câu này” và bấm “Lưu thay đổi”.</p><div class="detail-question">${esc2(g.content||'')}</div><div class="detail-options">${['A','B','C','D'].map(k=>`<div class="${String(g.correct_answer||'').toUpperCase()===k?'correct':''}"><b>${k}</b><span>${esc2(opts[k]||'')}</span></div>`).join('')}</div>${g.explanation?`<p class="hint"><b>Lời giải:</b> ${esc2(g.explanation)}</p>`:''}<p class="hint">Khi chấp nhận, câu này sẽ được lưu vào <b>Ngân hàng luyện tập – kiểm tra</b> để có mã câu hợp lệ cho frozen pool.</p><div class="form-actions"><button type="button" id="v122AiCancel" class="secondary">Hủy</button><button type="button" id="v122AiUse" class="primary">Dùng câu này</button></div></div>`);
  $1('#v122AiCancel')?.addEventListener('click',()=>{if(typeof closeModal==='function')closeModal();renderBuilder(ctx)});
  $1('#v122AiUse')?.addEventListener('click',()=>acceptAiQuestion(ctx,index,g));
}
async function acceptAiQuestion(ctx,index,g){
  const btn=$1('#v122AiUse');if(btn){btn.disabled=true;btn.textContent='Đang lưu câu…'}
  let createdId=null;
  try{
    const old=ctx.selected[index],now=new Date().toISOString();
    const qr=await db.from('questions').insert({subject_id:subjectId(),chapter_id:old.chapter_id,topic_id:old.topic_id,clo_id:old.clo_id,content:g.content,correct_answer:String(g.correct_answer||'').toUpperCase(),explanation:g.explanation||null,created_by:state.user.id,status:'active',question_scope:'practice',approval_status:'approved',approved_by:state.user.id,approved_at:now,origin_type:'gemini'}).select('id,subject_id,display_code,chapter_id,topic_id,clo_id,content,correct_answer,explanation,status,question_scope,approval_status').single();
    if(qr.error)throw qr.error;createdId=qr.data.id;
    const rows=['A','B','C','D'].map(k=>({question_id:createdId,option_key:k,content:g.options?.[k]||''}));
    const or=await db.from('question_options').insert(rows);if(or.error)throw or.error;
    const nq={...qr.data,question_options:rows};ctx.sets.questions.push(nq);ctx.selected[index]=nq;ctx.selectionDirty=true;
    if(typeof closeModal==='function')closeModal();renderBuilder(ctx);notify(`Đã dùng câu Gemini cho Câu ${index+1}. Bấm Lưu thay đổi để cập nhật bài.`);
  }catch(e){if(createdId){try{await db.from('questions').delete().eq('id',createdId)}catch{}}showError(e);if(btn){btn.disabled=false;btn.textContent='Dùng câu này'}}
}
function bindBuilder(ctx){
  const c=$1('#content');
  $1('#v122BuilderBack',c).onclick=()=>ctx.examId?openExamDetail(ctx.examId):exams(c);
  $1('#v122BuilderCancel',c).onclick=()=>ctx.examId?openExamDetail(ctx.examId):exams(c);
  $$1('[data-v122-setting]',c).forEach(el=>el.oninput=()=>{const k=el.dataset.v122Setting;ctx.settings[k]=el.type==='number'?+el.value:el.value});
  $$1('[data-v122-check]',c).forEach(el=>el.onchange=()=>ctx.settings[el.dataset.v122Check]=el.checked);
  $$1('[data-v122-chapter]',c).forEach(el=>el.onchange=()=>{const id=el.dataset.v122Chapter,topics=ctx.sets.topics.filter(t=>t.chapter_id===id);if(el.checked){ctx.selectedChapters.add(id);topics.forEach(t=>ctx.selectedTopics.add(t.id))}else{ctx.selectedChapters.delete(id);topics.forEach(t=>ctx.selectedTopics.delete(t.id))}ctx.selected=[];ctx.selectionDirty=true;cleanMatrix(ctx);renderBuilder(ctx)});
  $$1('[data-v122-topic]',c).forEach(el=>el.onchange=()=>{const id=el.dataset.v122Topic,ch=el.dataset.chapter;if(el.checked)ctx.selectedTopics.add(id);else ctx.selectedTopics.delete(id);const any=ctx.sets.topics.some(t=>t.chapter_id===ch&&ctx.selectedTopics.has(t.id));if(any)ctx.selectedChapters.add(ch);else ctx.selectedChapters.delete(ch);ctx.selected=[];ctx.selectionDirty=true;cleanMatrix(ctx);renderBuilder(ctx)});
  $$1('input[name="v122StructureMode"]',c).forEach(el=>el.onchange=()=>{ctx.structureMode=el.value;ctx.matrix={};ctx.selected=[];ctx.selectionDirty=true;renderBuilder(ctx)});
  $$1('.v122-matrix-input',c).forEach(el=>el.onchange=()=>{ctx.matrix[el.dataset.key]=Math.max(0,+el.value||0);ctx.selected=[];ctx.selectionDirty=true;renderBuilder(ctx)});
  $$1('[data-v122-replace]',c).forEach(el=>el.onclick=()=>replaceSelectedQuestion(ctx,+el.dataset.v122Replace));
  $$1('[data-v122-ai]',c).forEach(el=>el.onclick=()=>generateAiQuestion(ctx,+el.dataset.v122Ai,el));
  $1('#v122Draw',c).onclick=()=>{try{validateBuilder(ctx);ctx.selected=drawSelection(ctx);ctx.selectionDirty=true;renderBuilder(ctx);notify(`Đã rút ${ctx.selected.length} câu phù hợp ma trận`)}catch(e){showError(e)}};
...
async function saveBuilder(ctx){
  let createdId=null,saveBtn=$1('#v122Save');
  try{
    if(!ctx.settings.title.trim())throw new Error('Cần nhập tên bài kiểm tra');
    if(ctx.settings.opens_at&&ctx.settings.closes_at&&new Date(ctx.settings.closes_at)<=new Date(ctx.settings.opens_at))throw new Error('Thời gian đóng phải sau thời gian mở');
    const total=ctx.locked?(+ctx.exam?.total_questions||0):validateBuilder(ctx);
    if(!ctx.locked&&ctx.selected.length!==total)throw new Error('Cần nhấn “Rút câu hỏi” sau lần chỉnh ma trận cuối cùng');
    if(saveBtn){saveBtn.disabled=true;saveBtn.textContent='Đang lưu…'}
    const settings={title:ctx.settings.title.trim(),description:ctx.settings.description.trim()||null,duration_minutes:+ctx.settings.duration_minutes||30,max_attempts:+ctx.settings.max_attempts||1,question_mode:ctx.settings.question_mode,score_policy:ctx.settings.score_policy,opens_at:iso(ctx.settings.opens_at),closes_at:iso(ctx.settings.closes_at),show_answers:!!ctx.settings.show_answers,shuffle_questions:!!ctx.settings.shuffle_questions,shuffle_options:!!ctx.settings.shuffle_options,allow_ai_feedback:!!ctx.settings.allow_ai_feedback,counts_toward_grade:!!ctx.settings.counts_toward_grade};
    let examId=ctx.examId;
    if(!examId){
      const payload={...settings,subject_id:subjectId(),status:'draft',exam_type:'chapter_test',created_by:state.user.id,total_questions:total,chapter_ids:[...ctx.selectedChapters],topic_ids:[...ctx.selectedTopics],clo_counts:cloCounts(ctx),structure_mode:ctx.structureMode,question_blueprint:{version:1,source:'v12.2',matrix:{...ctx.matrix}}};
      const r=await db.from('exams').insert(payload).select('*').single();if(r.error)throw r.error;examId=r.data.id;createdId=examId;
    }else{
      const r=await db.from('exams').update(settings).eq('id',examId).select('*').single();if(r.error)throw r.error;
    }
    if(!ctx.locked){
      const pool=designPool(ctx).map(q=>poolSnapshot(q,ctx.sets)),blueprint={version:1,source:'v12.2',matrix:{...ctx.matrix}};
      const r=await db.rpc('replace_exam_design',{p_exam_id:examId,p_structure_mode:ctx.structureMode,p_blueprint:blueprint,p_chapter_ids:[...ctx.selectedChapters],p_topic_ids:[...ctx.selectedTopics],p_clo_counts:cloCounts(ctx),p_total_questions:total,p_pool:pool,p_selected:ctx.selected.map(q=>q.id)});
      if(r.error)throw r.error;
    }
    ctx.selectionDirty=false;notify(ctx.examId?'Đã lưu thay đổi':'Đã tạo bài kiểm tra');await openExamDetail(examId);
...
function scorePolicyLabel(v){return ({highest:'Điểm cao nhất',latest:'Lần cuối',average:'Trung bình các lần'}[v]||'Điểm cao nhất')}
function freshMetric(clos){return {attempts:0,examIds:new Set(),scoreSum:0,scoreCount:0,clos:Object.fromEntries((clos||[]).map(c=>[c.code,{correct:0,total:0}]))}}
function finishMetric(m,clos){const cloScores=(clos||[]).map(c=>{const x=m.clos[c.code]||{correct:0,total:0};return {code:c.code,description:c.description||'',correct:x.correct,total:x.total,score:x.total?x.correct*10/x.total:0}});return {attempts:m.attempts,examCount:m.examIds.size,gpa:m.scoreCount?m.scoreSum/m.scoreCount:0,clos:cloScores}}
async function loadOfficialResultBundle(){
  const sid=subjectId();
  const [{data:attempts,error:ae},{data:clos,error:ce},{data:exams,error:ee}]=await Promise.all([
    db.from('assessment_effective_attempts').select('id,exam_id,subject_id,student_id,attempt_number,started_at,submitted_at,score,score_policy').eq('subject_id',sid).order('submitted_at',{ascending:true}),
    db.from('clos').select('id,code,description').eq('subject_id',sid).order('code'),
    db.from('exams').select('id,title,score_policy,counts_toward_grade').eq('subject_id',sid).eq('counts_toward_grade',true).order('created_at',{ascending:true})
  ]);
  if(ae)throw ae;if(ce)throw ce;if(ee)throw ee;
  const ids=(attempts||[]).map(x=>x.id);let answers=[],questions=[];
  if(ids.length){const [ar,qr]=await Promise.all([db.from('student_answers').select('attempt_id,question_id,is_correct').in('attempt_id',ids),db.from('attempt_questions').select('attempt_id,question_id,clo_code').in('attempt_id',ids)]);if(ar.error)throw ar.error;if(qr.error)throw qr.error;answers=ar.data||[];questions=qr.data||[]}
  return {attempts:attempts||[],clos:clos||[],exams:exams||[],answers,questions};
}
function buildOfficialMetrics(bundle){
  const {attempts,clos,answers,questions}=bundle,answerMap=new Map(answers.map(a=>[`${a.attempt_id}|${a.question_id}`,a])),attemptMap=new Map(attempts.map(a=>[a.id,a]));
  const classMetric=freshMetric(clos),students=new Map();
  for(const a of attempts){const m=students.get(a.student_id)||freshMetric(clos);m.attempts++;m.examIds.add(a.exam_id);m.scoreSum+=Number(a.score||0);m.scoreCount++;students.set(a.student_id,m);classMetric.attempts++;classMetric.examIds.add(a.exam_id);classMetric.scoreSum+=Number(a.score||0);classMetric.scoreCount++}
  for(const q of questions){const a=attemptMap.get(q.attempt_id);if(!a||!q.clo_code)continue;const ans=answerMap.get(`${q.attempt_id}|${q.question_id}`),right=ans?.is_correct===true,sm=students.get(a.student_id);if(sm){sm.clos[q.clo_code]??={correct:0,total:0};sm.clos[q.clo_code].total++;if(right)sm.clos[q.clo_code].correct++}classMetric.clos[q.clo_code]??={correct:0,total:0};classMetric.clos[q.clo_code].total++;if(right)classMetric.clos[q.clo_code].correct++}
  const finishedStudents=new Map([...students].map(([id,m])=>[id,finishMetric(m,clos)])),classFinished=finishMetric(classMetric,clos),studentValues=[...finishedStudents.values()];
  classFinished.gpa=studentValues.length?studentValues.reduce((s,x)=>s+x.gpa,0)/studentValues.length:0;
  return {classMetric:classFinished,students:finishedStudents};
}
async function loadSubjectStudentProfiles(){
  const {data:members,error}=await db.from('subject_members').select('user_id,role').eq('subject_id',subjectId());if(error)throw error;
  const ids=[...new Set((members||[]).filter(x=>x.role==='student').map(x=>x.user_id))];if(!ids.length)return [];
  const {data,error:pe}=await db.from('profiles').select('id,full_name,mssv,email').in('id',ids);if(pe)throw pe;
  return (data||[]).sort((a,b)=>(a.full_name||a.email||'').localeCompare(b.full_name||b.email||'','vi'));
}
function resultCloCards(metric){return `<div class="clo-results">${(metric.clos||[]).map(x=>`<div class="${x.total&&x.score<4?'clo-below':''}"><b>${esc2(x.code)}</b><strong>${x.total?x.score.toFixed(2):'—'}</strong><span>${x.total?`${x.correct}/${x.total} câu đúng`:'Chưa có dữ liệu'}</span></div>`).join('')}</div>`}
function examPolicyNote(exams){return exams.length?exams.map(x=>`${esc2(x.title||'Bài kiểm tra')}: ${esc2(scorePolicyLabel(x.score_policy))}`).join(' · '):'Chưa có bài kiểm tra được tính vào CLO.'}
function aiAnalysisHtml(a){const actions=a?.recommendations||a?.next_actions||[];return `<div class="ai-analysis-v122"><p>${esc2(a?.summary||'')}</p>${a?.strengths?.length?`<h4>Điểm mạnh</h4><ul>${a.strengths.map(x=>`<li>${esc2(x)}</li>`).join('')}</ul>`:''}${a?.needs_improvement?.length?`<h4>Cần cải thiện</h4><ul>${a.needs_improvement.map(x=>`<li>${esc2(x)}</li>`).join('')}</ul>`:''}${actions.length?`<h4>Khuyến nghị</h4><ul>${actions.map(x=>`<li>${esc2(x)}</li>`).join('')}</ul>`:''}</div>`}
async function requestAssessmentAi(scope,studentId,button){
  ...
  try{const body={subject_id:subjectId(),scope};if(studentId)body.student_id=studentId;const {data,error}=await db.functions.invoke('analyze-assessment',{body});if(error)throw error;if(!data?.success)throw new Error(data?.error||'AI chưa tạo được nhận xét');const title=scope==='class'?'AI phân tích kết quả lớp':'AI nhận xét kết quả CLO';if(typeof openDrawer==='function')openDrawer(title,aiAnalysisHtml(data.analysis),null,{wide:true,eyebrow:'AI-CLO'});else if(typeof modal==='function')modal(title,aiAnalysisHtml(data.analysis));}
  catch(e){showError(e)}finally{if(button){button.disabled=false;button.textContent=old}}
}
function studentExamOfficialRows(bundle,studentId){return bundle.exams.map(exam=>{const rows=bundle.attempts.filter(a=>a.student_id===studentId&&a.exam_id===exam.id);if(!rows.length)return null;return {exam,attempts:rows.length,score:rows.reduce((s,x)=>s+Number(x.score||0),0)/rows.length}}).filter(Boolean)}
async function renderTeacherResults(c,bundle,metrics){
  const profiles=await loadSubjectStudentProfiles(),withData=profiles.filter(p=>metrics.students.has(p.id)),classM=metrics.classMetric;
  c.innerHTML=`<div class="assessment-results-v122"><div class="subpage-head"><div><small>KẾT QUẢ HỌC PHẦN</small><h3>Kết quả CLO</h3><p>Chỉ dùng các bài bật “Tính vào kết quả CLO học phần”; mỗi bài đã áp dụng đúng cách ghi nhận điểm.</p></div><button id="v122AiClass" class="ai-btn" ${classM.attempts?'':'disabled'}>✦ AI phân tích lớp</button></div><section class="panel"><div class="detail-grid"><div><small>SV có dữ liệu</small><b>${withData.length}/${profiles.length}</b></div><div><small>Bài được tính CLO</small><b>${bundle.exams.length}</b></div><div><small>Lượt chính thức</small><b>${classM.attempts}</b></div><div><small>GPA trung bình SV</small><b>${withData.length?classM.gpa.toFixed(2):'—'}</b></div></div><h4>CLO toàn lớp</h4>${resultCloCards(classM)}<details><summary>Quy tắc điểm đang áp dụng</summary><p class="hint">${examPolicyNote(bundle.exams)}</p></details></section><section class="panel"><div class="panel-head"><div><h3>Danh sách sinh viên</h3><p class="hint">Ngưỡng đạt CLO: 4.00/10.</p></div></div><div class="table-wrap"><table><thead><tr><th>STT</th><th>Sinh viên</th><th>GPA</th>${bundle.clos.map(x=>`<th>${esc2(x.code)}</th>`).join('')}<th>Lượt chính thức</th><th></th></tr></thead><tbody>${profiles.map((p,i)=>{const m=metrics.students.get(p.id);return `<tr><td>${i+1}</td><td><b>${esc2(p.full_name||p.email||'Sinh viên')}</b><br><small>${esc2(p.mssv||p.email||'')}</small></td><td>${m?`<b>${m.gpa.toFixed(2)}</b>`:'—'}</td>${bundle.clos.map(clo=>{const x=m?.clos.find(v=>v.code===clo.code);return `<td class="${x?.total&&x.score<4?'score-low':''}">${x?.total?x.score.toFixed(2):'—'}</td>`}).join('')}<td>${m?.attempts||0}</td><td>${m?.attempts?`<button class="ai-btn compact" data-v122-ai-student="${p.id}">✦ AI</button>`:'—'}</td></tr>`}).join('')||`<tr><td colspan="${bundle.clos.length+5}" class="empty">Chưa có sinh viên trong học phần.</td></tr>`}</tbody></table></div></section></div>`;
  $1('#v122AiClass',c)?.addEventListener('click',e=>requestAssessmentAi('class',null,e.currentTarget));$$1('[data-v122-ai-student]',c).forEach(b=>b.onclick=()=>requestAssessmentAi('student',b.dataset.v122AiStudent,b));
}
function renderStudentResults(c,bundle,metrics){
  const m=metrics.students.get(state.user.id)||finishMetric(freshMetric(bundle.clos),bundle.clos),rows=studentExamOfficialRows(bundle,state.user.id);
  ...
  $1('#v122AiMe',c)?.addEventListener('click',e=>requestAssessmentAi('student',state.user.id,e.currentTarget));
}
...
async function results(c){
  try{if(!subjectId()){c.innerHTML='<div class="panel empty">Chưa chọn học phần.</div>';return}if(!await schemaReady())return migrationNotice(c);const bundle=await loadOfficialResultBundle(),metrics=buildOfficialMetrics(bundle);if(isTeacher())await renderTeacherResults(c,bundle,metrics);else renderStudentResults(c,bundle,metrics)}catch(e){showError(e)}
}
