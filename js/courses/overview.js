/* AI-CLO PTITHCM V12.6.18 — canonical course overview owner with browser cache. */
(() => {
'use strict';
...
async function renderCourseOverview(c){
 ...
 if(r==='student'){
  ...
  cards=[
   ['Bài kiểm tra',student.examCount],
   ['Lượt đã nộp',student.submittedCount],
   ['Điểm trung bình',student.average===null?'—':student.average.toFixed(2)],
   ['CLO học phần',counts.cloCount]
  ];
  ...
  cards=[
   ['Chương · Chủ đề · CLO',`${counts.chapterCount} · ${counts.topicCount} · ${counts.cloCount}`],
   ['Câu luyện tập',counts.practiceCount],
   ['Câu đề thi',counts.secureCount],
   ['Sinh viên',counts.memberCount]
  ];
