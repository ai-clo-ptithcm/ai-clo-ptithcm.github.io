...
function navItems(){
 if(state.space==='system'){const r=role();return [
  ['dashboard','⌂','Tổng quan',true],['subjects','▣',role()==='admin'?'Quản lý học phần':isTeacher(role())?'Học phần phụ trách':'Học phần đang học',true],['questionBanks','?','Ngân hàng câu hỏi',r==='admin'],
...
function ongoingCourses(list){const today=new Date().toISOString().slice(0,10);return list.filter(s=>(!s.starts_on||s.starts_on<=today)&&(!s.ends_on||s.ends_on>=today))}
...
async function systemDashboard(c){
 ...
 let cards=[],intro='',action='',bankCount=0;
 if(r==='admin'){
  ...
  cards=[['Học phần đang diễn ra',ongoingCourses(state.subjects).length],['Ngân hàng câu hỏi',bankCount],['Người dùng',profiles.length],['Giảng viên',profiles.filter(p=>isTeacher(p.role)).length],['Sinh viên',profiles.filter(p=>p.role==='student').length]];
 ...
 const dashboardCourses=ongoingCourses(visible);c.innerHTML=`<div class="v109-dashboard"><section class="v109-hero"><div><small>TỔNG QUAN HỆ THỐNG</small><h3>Xin chào, ${esc(state.profile?.full_name||'bạn')}</h3><p>${esc(intro)}</p></div><span>${esc(roleLabel(r))}</span></section><div class="v109-stats">${cards.map(x=>stat(...x)).join('')}</div><section class="panel"><div class="panel-head"><h3>${r==='admin'?'Học phần đang diễn ra':esc(action)}</h3><button id="v109AllCourses" class="secondary">${esc(action)}</button></div><div class="v109-courses">${courseCards(dashboardCourses.slice(0,6))}</div></section></div>`;
...
window.users=async function(c){
 ...
 $('[data-member-tab="students"]',tabs).onclick=async()=>{sessionStorage.setItem(`aiclo:v109:members:${state.subjectId}`,'students');activate('students');body.innerHTML='<div class="panel">Đang tải danh sách sinh viên…</div>';await previousUsers(body)};
 $('[data-member-tab="teachers"]',tabs).onclick=async()=>{
  activate('teachers');sessionStorage.setItem(`aiclo:v109:members:${state.subjectId}`,'teachers');body.innerHTML='<div class="panel">Đang tải danh sách giảng viên…</div>';const memberships=await safe(()=>q('subject_members','id,user_id,role',x=>x.eq('subject_id',state.subjectId).in('role',['teacher','lecturer','giangvien'])),[]),ids=[...new Set(memberships.map(m=>m.user_id))],profiles=ids.length?await safe(()=>q('profiles','id,full_name,email,role,is_active,last_login_at',x=>x.in('id',ids).order('full_name')),[]):[],active=profiles.filter(p=>p.is_active!==false).length;
  body.innerHTML=`<div class="stats v1053-class-stats"><div class="stat"><small>Tổng giảng viên</small><b>${profiles.length}</b></div><div class="stat"><small>Đang hoạt động</small><b>${active}</b></div><div class="stat"><small>Đang bị khóa</small><b>${profiles.length-active}</b></div></div><div class="toolbar v1053-class-toolbar"><input id="v109TeacherSearch" placeholder="Tìm giảng viên…"><select id="v109TeacherStatus"><option value="all">Tất cả trạng thái</option><option value="active">Đang hoạt động</option><option value="locked">Đã khóa</option></select>${role()==='admin'?'<button id="v109ManageTeachers" class="primary">+ Thêm / bớt giảng viên</button>':''}</div><div class="panel table-wrap"><table><thead><tr><th>Họ tên</th><th>Email</th><th>Đăng nhập gần nhất</th><th>Trạng thái</th></tr></thead><tbody id="v109TeacherRows"></tbody></table></div>`;
  const draw=()=>{const s=$('#v109TeacherSearch').value.trim().toLowerCase(),sf=$('#v109TeacherStatus').value,list=profiles.filter(p=>!s||`${p.full_name||''} ${p.email||''}`.toLowerCase().includes(s)).filter(p=>sf==='all'||(sf==='active'?p.is_active!==false:p.is_active===false));$('#v109TeacherRows').innerHTML=list.map(p=>`<tr><td><b>${esc(p.full_name||'Chưa đặt tên')}</b></td><td>${esc(p.email||'—')}</td><td>${esc(memberTime(p.last_login_at))}</td><td><span class="badge ${p.is_active===false?'red':'green'}">${p.is_active===false?'Đã khóa':'Hoạt động'}</span></td></tr>`).join('')||'<tr><td colspan="4" class="empty">Không có giảng viên phù hợp.</td></tr>'};draw();$('#v109TeacherSearch').oninput=draw;$('#v109TeacherStatus').onchange=draw;if($('#v109ManageTeachers'))$('#v109ManageTeachers').onclick=()=>window.AICLO_COURSE_MEMBERS?.openManager?.(memberships,'teachers');
 };
 const savedMemberTab=sessionStorage.getItem(`aiclo:v109:members:${state.subjectId}`)||'students';if(savedMemberTab==='teachers')$('[data-member-tab="teachers"]',tabs).click();
};

function bindAssessmentTabs(c){
 const tabs=$('.v109-tabs',c),body=$('#v109AssessmentBody',c);if(!tabs||!body)return;
 let finalSection=$('.v102-final-list',body)||$('.v102-final-list',c);if(finalSection&&finalSection.parentElement!==body)body.appendChild(finalSection);
 const createFinal=$('#createFinalExam',c),head=finalSection&&$('.panel-head',finalSection);if(createFinal&&head)head.appendChild(createFinal);
 const online=[...body.children].filter(x=>x!==finalSection),show=tab=>{online.forEach(x=>{x.hidden=tab!=='online';x.classList.toggle('hidden',tab!=='online')});if(finalSection){finalSection.hidden=tab!=='final';finalSection.classList.toggle('hidden',tab!=='final')}$$('[data-assessment-tab]',tabs).forEach(b=>b.classList.toggle('active',b.dataset.assessmentTab===tab));sessionStorage.setItem(`aiclo:v109:assessment:${state.subjectId}`,tab)};
...
window.AICLO_FINAL_LAYER=Object.freeze({bindAssessmentTabs});
