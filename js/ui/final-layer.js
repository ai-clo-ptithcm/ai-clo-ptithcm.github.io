...
window.users=async function(c){
 ...
 $('[data-member-tab="teachers"]',tabs).onclick=async()=>{
  activate('teachers');sessionStorage.setItem(`aiclo:v109:members:${state.subjectId}`,'teachers');body.innerHTML='<div class="panel">Đang tải danh sách giảng viên…</div>';const memberships=await safe(()=>q('subject_members','id,user_id,role',x=>x.eq('subject_id',state.subjectId).in('role',['teacher','lecturer','giangvien'])),[]),ids=[...new Set(memberships.map(m=>m.user_id))],profiles=ids.length?await safe(()=>q('profiles','id,full_name,email,role,is_active',x=>x.in('id',ids).order('full_name')),[]):[],active=profiles.filter(p=>p.is_active!==false).length;
  body.innerHTML=`<div class="stats v1053-class-stats"><div class="stat"><small>Tổng giảng viên</small><b>${profiles.length}</b></div><div class="stat"><small>Đang hoạt động</small><b>${active}</b></div><div class="stat"><small>Đang bị khóa</small><b>${profiles.length-active}</b></div></div><div class="toolbar v1053-class-toolbar"><input id="v109TeacherSearch" placeholder="Tìm giảng viên…"><select id="v109TeacherStatus"><option value="all">Tất cả trạng thái</option><option value="active">Đang hoạt động</option><option value="locked">Đã khóa</option></select>${role()==='admin'?'<button id="v109ManageTeachers" class="primary">+ Thêm / bớt giảng viên</button>':''}</div><div class="panel table-wrap"><table><thead><tr><th>Họ tên</th><th>Email</th><th>Trạng thái</th></tr></thead><tbody id="v109TeacherRows"></tbody></table></div>`;
  const draw=()=>{const s=$('#v109TeacherSearch').value.trim().toLowerCase(),sf=$('#v109TeacherStatus').value,list=profiles.filter(p=>!s||`${p.full_name||''} ${p.email||''}`.toLowerCase().includes(s)).filter(p=>sf==='all'||(sf==='active'?p.is_active!==false:p.is_active===false));$('#v109TeacherRows').innerHTML=list.map(p=>`<tr><td><b>${esc(p.full_name||'Chưa đặt tên')}</b></td><td>${esc(p.email||'—')}</td><td><span class="badge ${p.is_active===false?'red':'green'}">${p.is_active===false?'Đã khóa':'Hoạt động'}</span></td></tr>`).join('')||'<tr><td colspan="3" class="empty">Không có giảng viên phù hợp.</td></tr>'};draw();$('#v109TeacherSearch').oninput=draw;$('#v109TeacherStatus').onchange=draw;if($('#v109ManageTeachers'))$('#v109ManageTeachers').onclick=()=>window.AICLO_COURSE_MEMBERS?.openManager?.(memberships,'teachers');
 };
