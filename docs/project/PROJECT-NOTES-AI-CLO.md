...
Cập nhật gần nhất: **06/09/2026 — V12.4.21**
...
- `css/system/dashboard.css` sở hữu **Dashboard hệ thống/học phần V10.9** (`v109-dashboard`, hero, KPI `v109-stats`, course cards và quick actions). Không đặt lại các selector Dashboard vào `css/ui/final-layer.css`.
- `css/system/profile.css` sở hữu cả **profile V10.8** và **profile/tài khoản V10.9 đang chạy** (`v109-profile`, `v109-security`, `v109-account-form`, password control); không đặt CSS hồ sơ/bảo mật tài khoản vào late layer.
- `css/questions/analysis.css` sở hữu **thống kê/phân tích câu hỏi**; `css/question-exam.css` sở hữu metadata/list/detail của Ngân hàng câu hỏi như `question-clo`; `css/courses/class-list.css` sở hữu action/trạng thái và UI thành viên của học phần (`last-login-cell`, `message-student`, `v109-member-tabs`, member summary/table compatibility).
...
- `css/exams/assessment-shared.css` sở hữu **Assessment runtime dùng chung**, gồm `v109-assessment-workspace` và `online-matrix-*`; `css/exams/detail-enhancements.css` sở hữu toàn bộ **trang Chi tiết bài kiểm tra/attempt table**, cả layout lẫn chrome; `css/exams/final-workflow.css` sở hữu compatibility của danh sách đề cuối kỳ như `v102-final-list`.
- `css/ui/final-layer.css` V12.4.21 chỉ còn **shared compatibility chrome** `v109-tabs` và `v109-workspace-head`. Không đưa Dashboard/Profile/Members/Assessment/Question Bank/Notification quay lại file này.
