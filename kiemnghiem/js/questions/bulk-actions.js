...
let currentBar=null,drag=null,enhanceQueued=false,observer=null;
...
function applyVisibleSelection(){document.querySelectorAll('#qrows [data-select-question]').forEach(check=>{check.checked=selected.has(check.dataset.selectQuestion)})}
function syncBar(){
...
async function filteredManageableQuestions(){
 ...
 if(typeof contentFilter==='function')query=contentFilter(query);else if(state.subjectId)query=query.eq('subject_id',state.subjectId);
 const {data,error}=await query;if(error)throw error;
 const search=String(document.querySelector('#qsearch')?.value||'').trim().toLowerCase(),chapter=document.querySelector('#qchapterFilter')?.value||'all',topic=document.querySelector('#qtopicFilter')?.value||'all',clo=document.querySelector('#qcloFilter')?.value||'all',approval=document.querySelector('#qapprovalFilter')?.value||'all',creator=document.querySelector('#qcreatorFilter')?.value||'all',bank=window.AICLO_V105?.activeBank?.()||'practice';
 return (data||[]).filter(x=>{
  ...
  if(chapter!=='all'&&x.chapter_id!==chapter)return false;if(topic!=='all'&&x.topic_id!==topic)return false;if(clo!=='all'&&x.clo_id!==clo)return false;if(approval!=='all'&&x.approval_status!==approval)return false;if(creator!=='all'&&x.created_by!==(creator==='mine'?state.user.id:creator))return false;
...
async function selectAllFiltered(button){
 ...
 try{const rows=await filteredManageableQuestions();selected.clear();rows.forEach(x=>selected.add(String(x.id)));syncBar();toast(`Đã chọn ${rows.length} câu trong kết quả lọc hiện tại`)}catch(ex){err(ex)}finally{button.disabled=false;button.textContent=old}
}
...
function modalMarkup(count){
 return `<form id="bulkQuestionActionForm" class="bulk-action-form">
  ...
  <div id="bulkAcademyWarning" class="bulk-action-warning" hidden>Chọn nguồn Học viện sẽ đưa câu vào Ngân hàng đề thi – bảo mật và trạng thái Chờ duyệt. Câu Học viện đã được xác nhận sẽ được giữ nguyên.</div>
...
function setGeometry(dialog,left,top,width,height){dialog.style.left=`${Math.round(left)}px`;dialog.style.top=`${Math.round(top)}px`;dialog.style.width=`${Math.round(width)}px`;dialog.style.height=`${Math.round(height)}px`;dialog.style.right='auto';dialog.style.bottom='auto';dialog.style.transform='none'}
function resetGeometry(dialog){if(matchMedia('(max-width:700px)').matches){['left','top','right','bottom','width','height','transform'].forEach(p=>dialog.style[p]='');return}const pad=16,width=Math.min(620,innerWidth-pad*2),height=Math.min(430,innerHeight-pad*2);setGeometry(dialog,(innerWidth-width)/2,(innerHeight-height)/2,width,height)}
function installDrag(dialog){
 ...
 dialog.addEventListener('close',()=>{drag=null;head.classList.remove('dragging');head.removeEventListener('pointerdown',down);head.removeEventListener('pointermove',move);head.removeEventListener('pointerup',up);head.removeEventListener('pointercancel',up);dialog.classList.remove('quick-edit-modal','bulk-actions-modal');['left','top','right','bottom','width','height','transform'].forEach(p=>dialog.style[p]='')},{once:true});
}

async function loadSelectedQuestions(){const list=ids();if(!list.length)return [];const {data,error}=await db.from('questions').select('id,origin_type,is_official,question_scope,approval_status').in('id',list);if(error)throw error;return data||[]}
function describeChanges(v){const parts=[];if(v.question_scope)parts.push(`Nơi lưu → ${scopeLabel(v.question_scope)}`);if(v.approval_status)parts.push(`Trạng thái → ${approvalLabel(v.approval_status)}`);if(v.origin_type)parts.push(`Nguồn → ${originLabel(v.origin_type)}`);return parts}
async function updateRows(ids,patch){if(!ids.length||!Object.keys(patch).length)return;const {error}=await db.from('questions').update({...patch,updated_at:new Date().toISOString()}).in('id',ids);if(error)throw error}
async function applyBulkChanges(values){
 ...
 const now=new Date().toISOString(),base={};
 ...
 const hasBase=Object.keys(base).length>0,official=rows.filter(x=>x.is_official),editable=rows.filter(x=>!x.is_official);let skippedOfficial=0;
 if(!hasBase&&!values.origin_type)throw new Error('Hãy chọn ít nhất một nội dung cần thay đổi.');
 if(values.origin_type==='academy'){
  if(!isAdmin())throw new Error('Chỉ Admin được hiệu chỉnh nguồn hàng loạt.');
  skippedOfficial=official.length;
  await updateRows(editable.map(x=>x.id),{origin_type:'academy',question_scope:'secure_exam',approval_status:'pending',status:'draft',approved_by:null,approved_at:null});
 }else{
  if(hasBase)await updateRows(rows.map(x=>x.id),base);
  if(values.origin_type){if(!isAdmin())throw new Error('Chỉ Admin được hiệu chỉnh nguồn hàng loạt.');skippedOfficial=official.length;await updateRows(editable.map(x=>x.id),{origin_type:values.origin_type})}
 }
 const updatedIds=new Set(values.origin_type==='academy'?editable.map(x=>x.id):[...(hasBase?rows.map(x=>x.id):[]),...(values.origin_type?editable.map(x=>x.id):[])]);
 window.logActivity?.('update','question',null,`Cập nhật hàng loạt ${updatedIds.size} câu: ${describeChanges(values).join(' · ')}`,'success',state.subjectId,{count:updatedIds.size,changes:values,skipped_official:skippedOfficial});
 return {updated:updatedIds.size,skippedOfficial};
}
...
async function openBulkWindow(){
 ...
 form.onsubmit=async e=>{
  e.preventDefault();const fd=new FormData(form),values={question_scope:String(fd.get('question_scope')||''),approval_status:String(fd.get('approval_status')||''),origin_type:String(fd.get('origin_type')||'')};if(source?.value==='academy'){values.question_scope='secure_exam';values.approval_status='pending'}
  ...
  const message=`Cập nhật ${rows.length} câu${officialSourceSkip?` · ${officialSourceSkip} câu Học viện chính thức sẽ giữ nguyên nguồn`:''}?\n${changes.join(' · ')}`;
...
document.addEventListener('change',e=>{if(e.isTrusted&&e.target?.matches?.('#qchapterFilter,#qtopicFilter,#qcloFilter,#qapprovalFilter,#qcreatorFilter'))clearSelection()},true);
...
function init(){const host=document.querySelector('#content');if(host&&!observer){observer=new MutationObserver(queueSync);observer.observe(host,{childList:true,subtree:true})}queueSync()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();

window.AICLO_QUESTION_BULK_SELECTION=Object.freeze({ids,clear:clearSelection,sync:syncBar,selectAll:()=>filteredManageableQuestions().then(rows=>{selected.clear();rows.forEach(x=>selected.add(String(x.id)));syncBar();return ids()})});
})();
