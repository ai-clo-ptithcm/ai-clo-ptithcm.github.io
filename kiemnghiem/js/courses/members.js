/* AI-CLO PTITHCM V12.6.19-kiemnghiem — canonical course members owner. */
(() => {
'use strict';
...
const canManageCourseMembers=()=>role()==='admin'||teacherRoles.includes(role());
...
async function openClassMembershipManager(members,memberType='students'){
 ...
 modal(`Thành viên · ${subject?.name||'Học phần'}`,`<div class="class-manager v1053-class-manager"><div class="class-column"><h4>Thêm tài khoản đã có vào lớp</h4><p class="hint">Chỉ chọn tài khoản đã tồn tại trong hệ thống. Trang này không tạo tài khoản mới.</p><input id="memberSearch" placeholder="Tìm tên, email, MSSV…"><div id="candidateList" class="member-list"></div><button id="addSelectedMembers" class="primary">+ Thêm vào lớp</button></div><div class="class-column"><h4>${memberType==='teachers'?'Giảng viên':'Sinh viên'} hiện tại (${scopedMembers.length})</h4><div class="member-list">${scopedMembers.map(m=>{let p=profiles.find(x=>x.id===m.user_id);return `<div class="member-item"><span><b>${esc(p?.full_name||m.user_id)}</b><small>${esc(p?.email||'')}${p?.mssv?` · ${esc(p.mssv)}`:''}</small></span><button class="danger" data-remove-member="${m.id}">Gỡ</button></div>`}).join('')||'<div class="empty">Chưa có thành viên.</div>'}</div></div></div>`);
 const draw=list=>$('#candidateList').innerHTML=list.map(p=>`<label class="member-item"><input type="checkbox" value="${p.id}"><span><b>${esc(p.full_name||'Chưa đặt tên')}</b><small>${esc(p.email||'')} · ${esc(p.mssv||roleLabel(p.role))}</small></span></label>`).join('')||'<div class="empty">Không còn tài khoản phù hợp để thêm.</div>';
 draw(eligible);
 ...
 $('#addSelectedMembers').onclick=async()=>{let ids=$$('#candidateList input:checked').map(x=>x.value);if(!ids.length)return toast('Chưa chọn tài khoản',true);let rows=profiles.filter(p=>ids.includes(p.id)).map(p=>({subject_id:state.subjectId,user_id:p.id,role:p.role}));let {error}=await db.from('subject_members').upsert(rows,{onConflict:'subject_id,user_id'});if(error)return err(error);closeModal();toast(`Đã thêm ${rows.length} thành viên vào lớp`);render()};
 $$('[data-remove-member]').forEach(b=>b.onclick=async()=>{if(!await confirmAction('Gỡ khỏi lớp','Gỡ tài khoản này khỏi học phần hiện tại? Tài khoản hệ thống sẽ không bị xóa.',{confirmLabel:'Gỡ khỏi lớp',danger:true}))return;let {error}=await db.from('subject_members').delete().eq('id',b.dataset.removeMember);if(error)return err(error);closeModal();toast('Đã gỡ thành viên khỏi lớp');render()});
}
...
function profileButton(p){
 return `<button type="button" class="user-name-link-v108" data-course-member-profile="${p.id}">${esc(p.full_name||'Chưa đặt tên')}</button>`;
}

function bindProfileLinks(root,profiles){
 $$('[data-course-member-profile]',root).forEach(b=>b.onclick=()=>{
  const p=profiles.find(x=>x.id===b.dataset.courseMemberProfile);
  if(p)window.AICLO_PROFILE?.openUserProfile?.(p);
 });
}
...
function renderStudents(body,members,profiles){
 ...
 body.innerHTML=`<div class="stats v1053-class-stats"><div class="stat"><small>Tổng sinh viên</small><b>${total}</b></div><div class="stat"><small>Đang hoạt động</small><b>${total-locked}</b></div><div class="stat"><small>Đang bị khóa</small><b>${locked}</b></div></div><div class="toolbar v1053-class-toolbar"><input id="classMemberSearch" placeholder="Tìm sinh viên…"><select id="classMemberStatus"><option value="all">Tất cả trạng thái</option><option value="active">Đang hoạt động</option><option value="locked">Đã khóa</option></select>${canManageCourseMembers()?'<button id="manageCurrentClass" class="primary">+ Thêm / bớt sinh viên</button>':''}</div><div class="panel table-wrap v1053-class-table"><table><thead><tr><th>Họ tên</th><th>Email / MSSV</th><th>Đăng nhập gần nhất</th><th>Trạng thái</th><th></th></tr></thead><tbody id="classMemberRows"></tbody></table></div>`;
 const draw=()=>{const s=$('#classMemberSearch').value.toLowerCase(),sf=$('#classMemberStatus').value,rows=list.filter(p=>[p.full_name,p.email,p.mssv].some(v=>String(v||'').toLowerCase().includes(s))).filter(p=>sf==='all'||(sf==='active'?p.is_active!==false:p.is_active===false));$('#classMemberRows').innerHTML=rows.map(p=>`<tr class="${p.is_active===false?'account-locked':''}"><td>${profileButton(p)}</td><td>${esc(p.email||'—')}<br><small>${esc(p.mssv||'')}</small></td><td>${esc(memberTime(p.last_login_at))}</td><td><span class="badge ${p.is_active===false?'red':'green'}">${p.is_active===false?'Đã khóa':'Hoạt động'}</span></td><td class="row-actions">${role()==='admin'?`<button class="danger" data-remove-class-member="${membership.get(p.id)?.id||''}">Gỡ</button>`:''}</td></tr>`).join('')||'<tr><td colspan="5" class="empty">Không có sinh viên phù hợp.</td></tr>';bindProfileLinks($('#classMemberRows'),profiles)};
 draw();$('#classMemberSearch').oninput=draw;$('#classMemberStatus').onchange=draw;if($('#manageCurrentClass'))$('#manageCurrentClass').onclick=()=>openClassMembershipManager(members,'students');
...
window.AICLO_COURSE_MEMBERS=Object.freeze({version:'12.6.19-kiemnghiem',render:renderCourseMembers,openManager:openClassMembershipManager});
})();
