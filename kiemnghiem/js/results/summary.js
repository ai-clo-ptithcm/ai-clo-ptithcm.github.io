...
function setupResultSorting(c,base,stats){
 const table=$('table',c),body=table?.tBodies?.[0],heads=table?.tHead?.rows?.[0]?.cells;if(!body||!heads)return {refresh(){}};
 let sort={key:'name',direction:'asc'};
 const keys=['name',null,'gpa',...base.clos.map(x=>`clo:${x.code}`),null];
 const labels=['Sinh viên',null,'GPA',...base.clos.map(x=>x.code),null];
 const rows=new Map();
 for(const p of base.profiles){const row=$(`[data-student-profile="${p.id}"]`,body)?.closest('tr');if(!row)continue;const s=stats.get(p.id),n=s?.attempts.length||0,gpa=n?s.scoreSum/n:null;row.dataset.studentId=p.id;row.dataset.sortName=p.full_name||'';row.dataset.sortGpa=gpa==null?'':String(gpa);rows.set(p.id,row);const cell=row.cells[2];if(cell&&gpa!=null&&gpa<4)cell.classList.add('result-score-low')}
 const value=(row,key)=>key==='name'?row.dataset.sortName:key==='gpa'?row.dataset.sortGpa:row.querySelector(`[data-sort-clo="${CSS.escape(key.slice(4))}"]`)?.dataset.sortValue||'';
 const drawIndicator=()=>keys.forEach((key,i)=>{const b=heads[i]?.querySelector('[data-result-sort]');if(b)b.dataset.direction=sort.key===key?sort.direction:''});
 const apply=()=>{const list=[...rows.values()].sort((a,b)=>{let av=value(a,sort.key),bv=value(b,sort.key),aMissing=av==='',bMissing=bv==='';if(aMissing!==bMissing)return aMissing?1:-1;if(aMissing)return 0;let result=sort.key==='name'?av.localeCompare(bv,'vi',{sensitivity:'base'}):Number(av)-Number(bv);return sort.direction==='asc'?result:-result});list.forEach(row=>body.append(row));drawIndicator()};
 keys.forEach((key,i)=>{if(!key||!heads[i])return;heads[i].innerHTML=`<button type="button" class="result-sort" data-result-sort="${esc(key)}">${esc(labels[i])}<span aria-hidden="true"></span></button>`;heads[i].querySelector('button').onclick=()=>{sort=sort.key===key?{key,direction:sort.direction==='asc'?'desc':'asc'}:{key,direction:key==='name'?'asc':'desc'};apply()}});
 base.clos.forEach(clo=>{for(const p of base.profiles){const cell=$(`[data-result-clo="${CSS.escape(p.id+'|'+clo.code)}"]`,body);if(cell)cell.dataset.sortClo=clo.code}});
...
async function fastTeacherResults(c){
 ...
 void (async()=>{
  try{
   ...
   for(const p of base.profiles){const m=details.studentClo.get(p.id)||new Map();for(const clo of base.clos){const cell=$(`[data-result-clo="${CSS.escape(p.id+'|'+clo.code)}"]`,c),x=m.get(clo.code),value=x?score(x.correct,x.total):null;if(cell){cell.textContent=value==null?'—':value.toFixed(2);cell.dataset.sortValue=value==null?'':String(value);cell.classList.toggle('result-score-low',value!=null&&value<4)}}}sorter.refresh();
