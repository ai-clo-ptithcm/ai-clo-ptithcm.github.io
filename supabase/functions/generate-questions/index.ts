  ...
  fetch: withSupabase(
    ...
    async (req, ctx) => {
      ...
      try {
        ...
        /*
         * Khi bật tránh trùng, đưa các câu đã có vào ngữ cảnh để Gemini tránh lặp.
        ...
        if (avoidDuplicates) {
          ...
          avoidanceQuestions = (existingQuestions || [])
...
hoặc dạng nhiệm vụ nhưng vẫn giữ đúng chương, chủ đề và CLO.
          ...
          : `
Giảng viên đã tắt chức năng đối chiếu ngân hàng trước khi sinh câu hỏi.
Không gửi nội dung câu hỏi cũ vào ngữ cảnh AI.
Các câu tạo trong chính phiên này vẫn phải khác nhau rõ ràng về cấu trúc hỏi,
dữ kiện chính và hướng giải; không chỉ thay vài con số.
            `.trim();
...
${additionalRequirements || "Không có."}
        ...
        return Response.json({
          ...
          context: {
            ...
            topics,
            clo,
            duplicate_avoidance_enabled: avoidDuplicates,
