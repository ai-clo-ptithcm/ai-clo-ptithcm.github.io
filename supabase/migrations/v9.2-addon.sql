...
alter table public.profiles add column if not exists locked_by uuid references public.profiles(id) on delete set null;
...
using (
 ...
 or exists(
   ...
   join public.subject_members learner on learner.subject_id=mine.subject_id
   where mine.user_id=auth.uid() and mine.role in ('teacher','lecturer','giangvien')
     and learner.user_id=activity_logs.user_id
     and activity_logs.subject_id=mine.subject_id
 )
);
...
begin
  ...
  if urole='student' then
    ...
    group by e.id,e.subject_id,e.title,e.closes_at
    having count(*)>0
    on conflict(user_id,dedupe_key) do nothing;

    insert into public.notifications(user_id,subject_id,category,severity,title,message,target_view,target_id,dedupe_key)
    select uid,e.subject_id,'clo','warning','CLO của lớp cần chú ý',aq.clo_code||' hiện có điểm quy đổi trung bình '||round(10.0*avg(case when sa.is_correct then 1 else 0 end),1)||'/10.','results',null,'class-clo:'||e.subject_id::text||':'||aq.clo_code||':'||week_key
    ...
    group by e.subject_id,aq.clo_code having 10.0*avg(case when sa.is_correct then 1 else 0 end)<4
    on conflict(user_id,dedupe_key) do nothing;
